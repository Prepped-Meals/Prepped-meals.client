...
const AdminPaymentPage = () => {
  ...
  const generatePDF = async () => {
    ...
    doc.text(`Most Used Payment Method: ${data.mostUsedPaymentMethod}`, 20, afterChartY + 10);
        ...
        <main className="p-10 space-y-10 overflow-y-auto">
          ...
          {loading ? (
            <div className="flex justify-center items-center h-64">
              <p className="text-gray-500 text-lg animate-pulse">Loading payment report...</p>
            </div>
          ) : error ? (
            <div className="flex justify-center items-center h-64">
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-xl hover:shadow-2xl transition-all">
                  ...
                  <ul className="space-y-4">
                    {data.topCustomers.map((customer, index) => (
                      <li key={index} className="flex justify-between items-center text-gray-600 font-medium">
                        <span>{customer.customer_name}</span>
                        <span className="text-blue-600 font-bold">Rs.{customer.totalPaid}</span>
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-xl hover:shadow-2xl transition-all">
                  ...
                  <p className="text-gray-600 text-xl font-bold">{data.mostUsedPaymentMethod}</p>
                </div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-xl hover:shadow-2xl transition-all">
                  ...
                  <p className="text-gray-600 text-xl font-bold">{data.busiestDay}</p>
                </div>
              </div>

              <div> 
                <p className="text-md text-gray-800 mt-50 h-100">This screen displays a weekly overview of payment activity, featuring the top 3 customers with the highest payment totals.
                It also highlights the most frequently used payment method and identifies the busiest day for customer payments during the week.</p>
              </div>
  
              {/* Bottom Action */}
              <div className="mt-10 text-center">
                <p className="text-gray-400 text-sm">
                  Last updated: {new Date().toLocaleDateString()} • All data is refreshed weekly
                </p>
              </div>
            </>
          )}
